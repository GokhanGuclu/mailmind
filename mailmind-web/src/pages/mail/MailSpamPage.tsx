...
import { readerFromInboxMock } from './mail-reader-model';

const SPAM_PAGE_SIZE = 50;

export function MailSpamPage() {
  const { language } = useUIContext();
  const copy = mailDashboardContent[language];
  const spamRows = copy.mock.spamMockRows;
  ...
  const totalPages = Math.max(1, Math.ceil(spamRows.length / SPAM_PAGE_SIZE));

  useEffect(() => {
    const max = Math.max(0, Math.ceil(spamRows.length / SPAM_PAGE_SIZE) - 1);
    setPageIndex((p) => Math.min(p, max));
  }, [spamRows.length]);

  const pageRows = useMemo(() => {
    const start = pageIndex * SPAM_PAGE_SIZE;
    return spamRows.slice(start, start + SPAM_PAGE_SIZE);
  }, [spamRows, pageIndex]);

  const pageIds = useMemo(() => pageRows.map((r) => r.id), [pageRows]);

  const openedRow = useMemo(
    () => (openedId ? spamRows.find((r) => r.id === openedId) ?? null : null),
    [openedId, spamRows],
  );
  ...
  const pageRangeLabel = useMemo(
    ...
    [copy.inboxPageRangeTemplate, copy.inboxPageRangeEmpty, pageIndex, spamRows.length],
  );
        ...
        <div className="mail-dash-widget__body mail-dash-widget__body--inbox mail-inbox-list__wrap">
          ...
          >
              ...
              <div className="mail-inbox-toolbar__leading">
                ...
                <div className="mail-inbox-toolbar__actions-morph">
                  ...
                  <div
                    ...
                    aria-hidden={!hasSelection}
                  >
                    <div className="mail-inbox-toolbar__bulk-actions mail-inbox-toolbar__bulk-actions--spam">
                      <button
                        type="button"
                        className="mail-inbox-toolbar__bulk-not-spam-btn"
                        aria-label={copy.spamBulkNotSpamLabel}
                        title={copy.spamBulkNotSpamLabel}
                      >
                        {copy.spamBulkNotSpamLabel}
                      </button>
                      <button
                        type="button"
            ...
            <div className="mail-dash-list-scroll">
              <ul className="mail-dash-widget__list mail-inbox-list mail-inbox-list--no-star" aria-label={copy.navSpam}>
                {pageRows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="mail-inbox-list__sender" title={row.from}>
                        {row.from}
                      </span>
              ...
              {openedRow ? (
                <MailMessageReader
                  model={readerFromInboxMock(openedRow)}
                  variant="spam"
                  copy={copy}

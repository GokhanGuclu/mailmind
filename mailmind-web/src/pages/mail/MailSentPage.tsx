...
export function MailSentPage() {
  ...
  const sentRows = copy.mock.sentMockRows;
  ...
  const totalPages = Math.max(1, Math.ceil(sentRows.length / SENT_PAGE_SIZE));

  useEffect(() => {
    const max = Math.max(0, Math.ceil(sentRows.length / SENT_PAGE_SIZE) - 1);
    setPageIndex((p) => Math.min(p, max));
  }, [sentRows.length]);

  const pageRows = useMemo(() => {
    const start = pageIndex * SENT_PAGE_SIZE;
    return sentRows.slice(start, start + SENT_PAGE_SIZE);
  }, [sentRows, pageIndex]);

  const pageIds = useMemo(() => pageRows.map((r) => r.id), [pageRows]);

  const openedRow = useMemo(
    () => (openedId ? sentRows.find((r) => r.id === openedId) ?? null : null),
    [openedId, sentRows],
  );
  ...
  const pageRangeLabel = useMemo(
    ...
    [copy.inboxPageRangeTemplate, copy.inboxPageRangeEmpty, pageIndex, sentRows.length],
  );
        ...
        <div className="mail-dash-widget__body mail-dash-widget__body--inbox mail-inbox-list__wrap">
          ...
          >
            ...
            <div className="mail-dash-list-scroll">
                ...
                {pageRows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="mail-inbox-list__sender mail-inbox__from--sent" title={row.from}>
                        {row.from}
                      </span>
              ...
              {openedRow ? (
                <MailMessageReader
                  model={readerFromInboxMock(openedRow, { recipientMode: true })}
                  variant="sent"
                  copy={copy}

...
export function StarredWidgetBody({ copy: _copy }: Props) {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await messagesApi.listStarred(accessToken, activeAccount.id, { limit: 8 });
  ...
  if (loading && messages.length === 0) {
    return (
      <div
        className="mail-dash-widget__body mail-dash-widget__body--inbox"
        style={{ opacity: 0.4, padding: 16, textAlign: 'center', fontSize: '0.8125rem' }}
      >
  ...
  if (messages.length === 0) {
    return (
      <div
        ...
        style={{ opacity: 0.4, padding: 16, textAlign: 'center', fontSize: '0.8125rem' }}
      >
        {language === 'tr' ? 'Yıldızlı mail yok.' : 'No starred mail.'}
      </div>
    );
  }
